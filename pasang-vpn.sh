...
# 1. Input Data
# Check if already set via env (for automated testing)
if [ -z "$IP_VPN" ]; then
...
# We use string replacement with actual values to avoid template literal issues during injection
...
// 3. Inject Auto-Detect VPN logic
const vpnLogic = \`
    try {
