...
case "$os_id" in
    ubuntu)
        case "$os_version" in
            20.04|22.04|24.04) os_supported=true ;;
        esac
        ;;
    debian)
        case "$os_version" in
            11|12) os_supported=true ;;
        esac
        ;;
esac
...
if [[ "$os_supported" == "true" ]]; then
    ...
    echo -e "${EROR} OS Not Supported ( ${YELLOW}${os_pretty}${NC} )"
    echo -e "${EROR} Supported: Ubuntu 20.04/22.04/24.04, Debian 11/12"
    exit 1
fi
...
if [[ $today < $Exp1 ]]; then
    ...
    sts="${Error}"
    echo -e "${EROR} License has expired!"
    exit 1
fi
clear
